...
class ComposeEmailForm(FlaskForm):
    receiver = StringField('Receiver', validators=[DataRequired(), Email()])
    message = StringField('Message', validators=[DataRequired()])
    save_as_draft = StringField('Save as Draft')
class LoginForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired()])
    password = PasswordField('Password', validators=[DataRequired()])
...
class ReauthForm(FlaskForm):
    password = PasswordField('Password', validators=[DataRequired()])  # Corrected line
...
def register():
    ...
    form = RegisterForm()  # Use the RegisterForm

    if request.method == 'POST' and form.validate_on_submit():  # Use form validation
        ...
        if User.query.filter_by(username=username).first():
            ...
            return render_template('register.html', form=form)  # Pass form back to template
    ...
    if request.method == 'POST' and form.validate_on_submit():  # Use form validation
        ...
        if user:
            ...
            if not allowed:
                ...
                return render_template('login.html', form=form)  # Pass form back to template
...
def verify_2fa():
    ...
    form = Verify2FAForm()  # Create the form instance

    if request.method == 'POST' and form.validate_on_submit():
        input_code = form.code.data  # Get the 2FA code from the form
    ...
    return render_template('verify_2fa.html', form=form)  # Pass the form to the template
...
# Define the reauthentication form
class ReauthForm(FlaskForm):
    password = PasswordField('Password', validators=[DataRequired()])
...
def reauth():
    ...
    return render_template('reauth.html', form=form)  # Pass the form to the template
...
@login_required
def compose():
    form = ComposeEmailForm()  # Create ComposeEmailForm instance

    if request.method == 'POST' and form.validate_on_submit():
        receiver = form.receiver.data
        message = form.message.data
        save_as_draft = form.save_as_draft.data
    ...
    return render_template('compose_email.html', form=form)
